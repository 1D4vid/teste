return function(env)
    ...
    -- =========================================================================
    -- ESCOPO DE SEGURANÇA E CACHE DE ALTA PERFORMANCE
    -- =========================================================================
    ...
    pcall(function()
        table.insert(globalConnections, Players.PlayerAdded:Connect(updatePlayersCache))
        table.insert(globalConnections, Players.PlayerRemoving:Connect(updatePlayersCache))
    end)
        ...
        if state then
            local function createProgressBar(parent)
                ...
                pcall(function()
                    if currentComputerStyle == "Default" or currentComputerStyle == "Style 1" then
                        ...
                        track.BackgroundColor3 = (currentComputerStyle == "Default") and Color3.fromRGB(20, 20, 25) or Color3.fromRGB(30, 30, 30)
                        ...
                        bar.BackgroundColor3 = (currentComputerStyle == "Default") and Color3.fromRGB(0, 180, 255) or Color3.fromRGB(255, 255, 255)
            ...
            local function setupComputer(tableModel)
                pcall(function()
                    ...
                    if not billboard or not bar or not text then return end
    ...
    Library:CreateToggle(Page, "Door Progress", false, function(state)
        ...
        if state then
            ...
            local function createDoorHUD(parent)
                local billboard, fill, text, bgBar
                pcall(function()
    ...
    Library:CreateToggle(Page, "ExitDoor Progress", false, function(state)
        ...
        if state then
            ...
            folder.Name = guiName
            pcall(function() folder.Parent = targetGuiParent end)
            ...
            task.spawn(function()
                pcall(function()
                    ...
                    for i = 1, #workspaceDescendants do
                        if not exitDoorActive then break end
            ...
            ExitDoorConn = task.spawn(function()
                while exitDoorActive and task.wait(0.15) do 
                    pcall(function()
                        local openingNow = {}
    ...
    Library:CreateToggle(Page, "GetUp Timer", false, function(state)
        ...
        if state then
            ...
            pcall(function() getupGui.Parent = uiParent end)

            local function billboard(p, head)
                local timeLbl
                pcall(function()
                    if p == LocalPlayer or not head then return end 
                    local old = head:FindFirstChild("RC")
                    if old then old:Destroy() end
                    
                    if hideHeadGetUp then return end 
                ...
                if character then
                    ...
                    if humanoidRootPart then
                        ...
                        if not billboard then
                            ...
                            innerLabel.Name = "BeastPowerLabel"
                            innerLabel.Size = UDim2.new(1, 0, 1, 0)
                            innerLabel.BackgroundTransparency = 1
                            innerLabel.Font = Enum.Font.Arcade
                            innerLabel.TextSize = 20
                            innerLabel.Text = ""
                            innerLabel.TextStrokeTransparency = 0.5
                            innerLabel.TextColor3 = Color3.new(1, 1, 1)
                            innerLabel.TextStrokeColor3 = Color3.new(0, 0, 0)
                            innerLabel.Parent = billboard
                            
                            billboard.Parent = humanoidRootPart
                        end
                        label = billboard:FindFirstChild("BeastPowerLabel")
                    end
                end
            end)
            return label
    ...
    Library:CreateToggle(Page, "Life Timer", false, function(state)
        ...
        if state then
            ...
            local function getLifeTimerTarget(char)
                local part, offset = nil, Vector3.new(0, 0, 0)
                pcall(function()
